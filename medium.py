...
class TypingTest:
    ...
    def __init__(test):
        ...
        test.text = open("med_text.txt", "r").read().split("\n")
        ...
        test.T = tk.Label(test.frame, text="MEDIUM", font=("Helventica", 24))
        test.T.grid(row=0, column=0, columnspan=2, padx=5, pady=10)
        
        # creating a lable
        test.choice = random.choice(test.text)
        test.sample_label = tk.Label(test.frame, text=test.choice, font=("Helventica", 17))
